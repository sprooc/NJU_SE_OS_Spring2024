...
int print(char *str, uint32_t len);
...
FileDir *dfs_read(RootEntry *entry, FileDir *parent) {
  ...
  new_dir_file->abs_path = malloc(strlen(parent->abs_path) + 12);
  strcpy(new_dir_file->abs_path, parent->abs_path);
  int len = 8;
  while (entry->DIR_Name[len - 1] == ' ') {
    len--;
  }
  strncat(new_dir_file->abs_path, entry->DIR_Name, len);
  if (entry->DIR_Name[8] != ' ') {
    strcat(new_dir_file->abs_path, ".");
    int len = 3;
    while (entry->DIR_Name[7 + len] == ' ') {
      len--;
    }
    strncat(new_dir_file->abs_path, entry->DIR_Name + 8, len);
  }
  if (entry->DIR_Attr == TYPE_DIR) {
    strcat(new_dir_file->abs_path, "/");
  }
  new_dir_file->parent = parent;
  new_dir_file->size = entry->DIR_FileSize;
  new_dir_file->buf = read_secs(entry->DIR_FstClus, &clus_cnt);
  strncpy(new_dir_file->name, entry->DIR_Name, 11);
#ifdef DEBUG
  printf("%.11s\n", new_dir_file->name);
#endif
  if (entry->DIR_Attr == TYPE_FILE) {
...
void dfs_free(FileDir *root) {
  if (root->parent && root->abs_path) {
  free(root->abs_path);
  }
  if (root->parent && root->buf) {
    free(root->buf);
  }
  for (int i = 0; i < root->children_size; i++) {
    dfs_free(root->children[i]);
  }
  if (root->parent) {
    free(root);
  }
...
void print_dir(FileDir *dir, uint32_t cmd_flag) {
  if (cmd_flag == 0) {
    int len = strlen(dir->abs_path);
    print(dir->abs_path, len);
    print(":\n", 2);
    if (dir->parent) {
      print(red, 6);
      print(".  ..  ", 7);
    ...
    for (int i = 0; i < dir->children_size; i++) {
      if (dir->children[i]->type == TYPE_DIR) {
        print_dir(dir->children[i], cmd_flag);
      }
    }
  } else if (cmd_flag == DETAIL) {

    print(dir->abs_path, strlen(dir->abs_path));
    print(" ", 1);
...
void print_file(FileDir *file) { print(file->buf, strlen(file->buf)); }

int main(int arg, char *argv[]) {
  if (arg < 2) {
    print("Usage: ", 7);
    print(argv[0], strlen(argv[0]));
    print(" <filename>\n", 12);
    return 1;
  }
  ...
  while (1) {
    ...
    if (cmd_type == CMD_LS) {
      ...
      FileDir *target_dir = find_dir(target_path, &root);
      if (!target_dir) {
        FileDir *ls_file = find_file(target_path, &root);
        if (ls_file) {
          print(ls_file->abs_path, strlen(ls_file->abs_path));
          print("\n", 1);
        } else {
          print("No such directory\n", 18);
        }
      } else {
        print_dir(target_dir, cmd_flag);
      }
    } else {
